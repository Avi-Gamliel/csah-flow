...
auth.onAuthStateChanged(user => {
    ...
    if (user) {
        ...
        function loadYearly(d) {
            ...
            TotalCashout.textContent = ' ' + d[1].out.toFixed(2);
            TotalCashin.textContent = ' ' + d[1].in.toFixed(2);
            ...
            // TotalCashout.textContent = d.out.toFixed(2);
            // TotalCashin.textContent = d.in.toFixed(2);
        ...
        function TABLE_PAGE_GET_DATA() {
            ...
            database.ref('/Users/' + user.uid + '/cashflow/').orderByChild('number').once('value').then(snapshot => {
                ...
                for (var i = 0; i < keys.length; i++) {
                    ...
                    // console.log(temp, type, date, currectnum);
                            ...
                            if (uid == "chooseMonthly") {
                                ...
                                database.ref('/Users/' + user.uid + '/monthly/').once('value').then(snapshot => {
                                    ...
                                    for (var i = 0; i < keys.length; i++) {
                                        ...
                                        var type = currentObject.type;
                                        var date = currentObject.date;
                                        var inData = currentObject.in;
                                        var outData = currentObject.out;
                                        ...
                                        console.log(id, obg, currectLimit);
                                        // var keysmonth = Object.keys(montharr);
                                        // for (var i = 0; i < keysmonth.length; i++) {
                                        //     var uis = keysmonth[i];
                                        //     console.log(montharr[uis]);
                                        //     console.log(montharr[uis] == id);

                                        //     if (montharr[uis] === id) {
                                        //         console.log(montharr[uis], id);
                                        //         index[i] = [i, obg, currectLimit];
                                        //     }
                                        // }
                                        index.push([id, obg, currectLimit]);
        ...
        function HOME_PAGE_GET_DATA_FOR_WEEKLY() {
            ...
            database.ref('/Users/' + user.uid + '/cashflow/').orderByChild('number').on('value', function (snapshot) {
                ...
                // groupChartHome.forEach(function (y) {
                //     if (y.checked) {
                //         var W = y.id;
                //         console.log(W);
                //         if (W == 'weeklyChart') {
                            ...
                            database.ref('/Users/' + user.uid + '/monthly/').child(id).child(type).on('value', function (snapshot) {
                                ...
                                for (var i = 0; i < nkeys.length; i++) {
                                    ...
                                    if (type == 'in') {
                                        ...
                                        if (month == id) {
                                            countin += Number(obg.totalCash);
                                            var num = formatNumber(countin.toFixed(2));
                                            var split = num.split(".");
                                            let spannum = document.createElement("span");
                                            spannum.setAttribute("class", "decimel");

                                            spannum.textContent = split[1];

                                            num_incomingMonth.textContent = split[0] + ".";
                                            num_incomingMonth.appendChild(spannum)

                                            var balance = countin - countout;
                                            var numBalance = formatNumber(balance.toFixed(2));
                                            var splitBalance = numBalance.split(".");
                                            let spannumbal = document.createElement("span");
                                            spannumbal.setAttribute("class", "decimel");

                                            spannumbal.textContent = splitBalance[1];

                                            // console.log(balance);
                                            balanceMonth.textContent = splitBalance[0] + '.';
                                            balanceMonth.appendChild(spannumbal);
                                        ...
                                        if (month == id) {
                                            var num = formatNumber(countout.toFixed(2));
                                            var split = num.split(".");
                                            let spannum = document.createElement("span");
                                            spannum.setAttribute("class", "decimel");
                                            ...
                                            num_expendMonth.textContent = split[0] + ".";
                                            num_expendMonth.appendChild(spannum)
