    ...
    database.ref('/monthly/').on('value', function (snapshot) {
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            database.ref('/monthly/').child(id).on('value', function (snapshot) {
                ...
                data = [final[0], incomingB, outcomngB];
...
//     var getdata = await getDataYearly();
...
//     const getdatab = await getDataYearly();
//     var m = getdatab;
...
//         bars: 'vertical',
//         series: {
//             0: { axis: 'distance', color: 'red' },
//             1: { axis: 'distance', color: '#00cc00' }
//         },
//         axes: {
//             x: {
...
//     const getdatac = await getDataYearly();  // Some raw data (not necessarily accurate)
//     var m = getdatac;
//     m.forEach(snaphot => {
//         console.log(snaphot);
...
function drawStuff(m) {
    ...
    var options = {
        ...
        series: {
            ...
            1: { axis: 'distance', color: '#00cc00' } // Bind series 1 to an axis named 'brightness'.
        },
        axes: {
            x: {
                distance: { label: 'parsecs' }, // Bottom x-axis.
