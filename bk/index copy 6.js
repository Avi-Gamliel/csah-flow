...
function laodUpData(I, doc, S, A) {
    ...
    TotalCashAfter.textContent = formatNumber(S) + ' ש"ח ';
...
// var numCorrectDay = Date.parse(pastedate);
// console.log(numCorrectDay);


function laodUpWeekData(I, doc, S, A) {
    // console.log(I, doc, S, A);
    ...
    console.log(numd, dayCorrect, splitedate[0], monthCorrect);
    ...
    if (numd > dayCorrect && numd < limit && splitedate[0] == monthCorrect) {
        ...
        for (var i = 0; i < mainArray.length; i++) {
            ...
            console.log(date, title, type, way, cash)
...
database.ref('/cashflow/').orderByChild('number').on('value', function (snapshot) {
    ...
    for (var i = 1; i < keys.length + 1; i++) {
        ...
        if (subdate > newdateParse) {
            ...
            Status.textContent = formatNumber(postArray[keys[i]].after) + ' ש"ח ';
            currectCash.textContent = formatNumber(postArray[keys[i]].after) + ' ש"ח ';
            ...
            if (newBalance < 0) {
                ...
                // console.log(new_num, newBalance);
        ...
        laodUpWeekData(id, currentObject, StatusCash, currectLimit)
...
//         // IncomingCash.textContent = formatNumber(incomingB);
//         // outcomingCash.textContent = formatNumber(outcomngB);
//     }
...
//     // google.charts.setOnLoadCallback(drawStuff(chartmap));
//     // google.charts.setOnLoadCallback(drawVisualization(chartmap));
//     // google.charts.setOnLoadCallback(drawChart(chartmap));
//     return chartmap;
...
// // google.charts.setOnLoadCallback(drawStuff(chartmap));
// google.charts.setOnLoadCallback(drawVisualization(montharr));
// // google.charts.setOnLoadCallback(drawChart(chartmap));
// return chartmap;
let MainArray = [[]];
...
//         MainArray.push([y, trimTitle, in_out, typePay, cash]);
...
//                         MainArray.push([z, titleSub, inOrOut, type, totalcash]);
...
//         var date = MainArray[i][0];
//         var title = MainArray[i][1].trim();


//         var in_out = MainArray[i][2];
//         var payWay = MainArray[i][3];
...
//         if (checkarray.indexOf(title) == -1) {

//             checkarray.push(title);
//             sumArray.push([title, sum]);
...
//     // var chartb = new google.visualization.ComboChart(document.getElementById('dual_x_div'));
